...
//			if(flagOfBoundaryCondition[jParticle]==GHOST_OR_DUMMY)continue;
			if (flagOfBoundaryCondition[jParticle] != INNER_PARTICLE)continue;
...
void
MATHE_multiplySquareMatrixByVector( int    totalNum
			     ,double *answer
			     ,double **matrix
			     ,double *vector) {
	...
	for (iParticle = 0; iParticle < totalNum; iParticle++) {
		tmp = 0.0;
		for (jParticle = 0; jParticle < totalNum; jParticle) {
			tmp += matrix[iParticle][jParticle] * vector[jParticle];
		}
		answer[iParticle] = tmp;
	}
