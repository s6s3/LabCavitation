...
Bubble_calculateDiameter(int iParticle){
    ...
    if(particle.numberOfBubbles[iParticle]>0.0){
		...
		if (voidrate < 0) {
			//voidrate = 0;//todo
			//•‰ˆ³‚ðƒ[ƒ‚Æ‚µ‚Äˆµ‚Á‚½ê‡‚Ì‚É‡‚í‚¹‚é
			voidrate = (particle.moleOfBubbles[iParticle] * physicalProperty.gasConstant*physicalProperty.temperature) / ((physicalProperty.headPressure)*volume);
