  ...
  if(timer.iTimeStep_forcedMotion!=-1){
     if(parameter.flagOfOutputOfTorqueFile == ON){
        if(flag==0){
           ...
           power+=FORCEDMOTION_TorqueCalculationForStirredTank(1);
        }else{
           fp1 = FORCEDMOTION_setTorqueCalculationFile();
           power+=FORCEDMOTION_TorqueCalculationForStirredTank(3);
        }
     }
  }
...
double
FORCEDMOTION_TorqueCalculationForStirredTank(int numberOfStatus){
