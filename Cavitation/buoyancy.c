...
Buoyancy_calculateBuyancy( void ){
    ...
    for(iParticle= 0; iParticle < particle.totalNumber; iParticle++){
		...
		if (voidrate < 0) {
			//todo
			voidrate = (particle.moleOfBubbles[iParticle] * physicalProperty.gasConstant*physicalProperty.temperature) / ((physicalProperty.headPressure)*(Bubble_calculateVolume(particle.averageDistance)));
